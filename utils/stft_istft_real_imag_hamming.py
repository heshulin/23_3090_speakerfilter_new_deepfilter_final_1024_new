...
class STFT(torch.nn.Module):
    def __init__(self, filter_length=1024, hop_length=512):
        ...
        inverse_basis = inverse_basis #* torch.FloatTensor(window)
        self.register_buffer('forward_basis', forward_basis.float())
        self.register_buffer('inverse_basis', inverse_basis.float())

        self.register_buffer('part_1', torch.FloatTensor(window[:self.hop_length]))
        self.register_buffer('part_2', torch.FloatTensor(window[:self.hop_length] + window[self.hop_length:]))
        self.register_buffer('part_3', torch.FloatTensor(window[self.hop_length:]))
    def transform(self, input_data):
        ...
        return res
    def win_all_len(self, Frm_num):
        assert Frm_num >= 3
        return torch.cat([self.part_1, self.part_2.repeat(Frm_num - 1), self.part_3], 0)

        # return torch.cat([self.part_1, self.part_2.repeat(1, Frm_num - 2), self.part_3], 1)
    def inverse(self, stft_res):
        ...
        return inverse_transform[:, 0, :] / self.win_all_len(real_part.shape[2])
